...
func TestLoadDefaultsToSafeDryRun(t *testing.T) {
	...
	settings := Load()
	if settings.HTTPAddress != ":8080" || settings.DBPath != "data/crowsnest.sqlite3" || !settings.DryRun || !settings.ReconcileDryRun || settings.IdentitySyncEnabled || !settings.IdentitySyncDryRun {
...
func TestLoadSeparatesWebhookAndReconcileSettings(t *testing.T) {
	...
	settings := Load()
	if settings.WebhookSecret != "incoming-token" || settings.GitLabHookToken != "webhook-token" || settings.GitLabBaseURL != "https://gitlab.example.com" || settings.ReconcileDryRun || !settings.IdentitySyncEnabled || settings.IdentitySyncDryRun || settings.IdentitySyncInterval != 30*time.Minute {
		t.Fatalf("settings = %#v", settings)
	}
}
...
func clearConfigEnvironment(t *testing.T) {
	...
	for _, key := range []string{
		...
		"CROWSNEST_DELIVERY_POLL_INTERVAL", "CROWSNEST_RECONCILE_INTERVAL", "CROWSNEST_IDENTITY_SYNC_ENABLED", "CROWSNEST_IDENTITY_SYNC_DRY_RUN", "CROWSNEST_IDENTITY_SYNC_INTERVAL", "CROWSNEST_ALLOWED_EMAIL_DOMAINS", "CROWSNEST_GITLAB_BASE_URL",
